...
def extract_rider_info(table):
    ...
    for t in table:
        ...
        try:
            name = t.find(
                "span", {"class": "surname qa_table_rider_surname"}
            ).text.replace("\n", "")
        except:
            name = None
        try:
        ...
        try:
            const_class = (
                "table_item constructor qa_table_constructor d-none d-sm-table-cell"
            )
        ...
        try:
            ride_class = (
                "table_item category qa_table_category d-table-cell text-right pr-8"
            )
